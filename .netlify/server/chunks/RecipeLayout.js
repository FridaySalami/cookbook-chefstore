...
function RecipeLayout($$payload, $$props) {
  ...
  if (image) {
    ...
    if (responsiveImage) {
      $$payload.out += "<!--[-->";
      $$payload.out += `<picture><source${attr("srcset", responsiveImage.srcset)}${attr("sizes", responsiveImage.sizes)} type="image/webp"> <img${attr("src", `/images/recipes/category-name/recipe-name/resized/${slug}-800w.webp`)}${attr("alt", title)} class="aspect-video w-full object-cover" width="800" height="450" loading="lazy" onload="this.__e=event" onerror="this.__e=event"></picture>`;
    } else {
      $$payload.out += "<!--[!-->";
